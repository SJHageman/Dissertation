...
\begin{equation}
	\label{eqn:dod_sum}
	\dod_{tot}(\omega) = \sum_{n} A_n W_n(\omega)\big(\sigma_{\mathrm{on},n}(\omega) -\sigma_{\mathrm{off},n}(\omega)\big)
\end{equation} 
where $A_n$ is an amplitude that is used to fit to the measured $\dod$ and $W_n(\omega)$ is a window function that limits the range of the calculated $\sigma_n$ to $\pm3\Gamma_n$.  The window function is applied because the form of the dipole used in the DCM inherently assumes a flat continuum, and this approximation is only valid near the resonance \cite{chuTheoryUltrafastAutoionization2010}.  The resulting calculation is shown in figure \ref{fig:a1_t_intensity} (b).
